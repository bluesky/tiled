...
with Context.from_app(build_app(catalog)) as context:
    ...
    primary = (
        client.create_container(
            "runs", specs=[Spec("CatalogOfBlueskyRuns", version="3.0")]
        )
...
with Context.from_app(build_app(catalog)) as context:
    ...
    stream_called_streams = runs_node[uid]["streams"].create_container(
        "streams", specs=[Spec("BlueskyEventStream", version="3.0")]
    )
    ...
    empty_run = runs_node.create_container(
        "empty_run", specs=[Spec("BlueskyRun", version="3.0")]
    )
    empty_run.create_container("streams")

    # 4. Create a BlueskyRun with no streams node containing an array -- should not be deleted
    non_empty_run = runs_node.create_container(
        "non_empty_run", specs=[Spec("BlueskyRun", version="3.0")]
    )
    non_empty_run.create_container("streams").write_array(
        np.random.randn(3, 4), key="arr1"
    )

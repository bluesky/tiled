...
class AuthenticationProviderSpec(BaseModel):
    ...
    def into_auth_entry(
        ...
        if not isinstance(auth, (InternalAuthenticator, ExternalAuthenticator)):
            raise ValueError(
                f"Type {self.authenticator.__class__} is not a known authenticator type"
            )
...
class Config(BaseModel):
    ...
    def serialization_registry(self) -> SerializationRegistry:
        ...
        for ext, media_type in self.file_extensions:
    ...
    def validation_registry(self) -> ValidationRegistry:
        ...
        for spec in self.specs:
            base.register(spec.spec, spec.validator or _no_op_validator)
        return base

...
_UPSERT_ENTITY = f"""
mutation($input: CreateEntityInput!) {{
  upsertEntity(input: $input) {{ {_ENTITY_FIELDS} }}
}}
"""
_UPDATE_ENTITY = f"""
...
class GraphClient:
    ...
    def upsert_entity(
        ...
        *,
        ...
        """Get-or-create a node-bound entity, keyed on (node, kind, name).

        Idempotent: returns the existing entity if one already matches, else
        creates it (refreshing `uri`/`properties` to the values given). Unlike
        `create_entity`, this never raises `EntityExistsError`. `node_path_parts`
        is required, since only node-bound entities have a natural key to
        deduplicate on; use `create_entity`/`make_entity` for external entities.
        """
        input: dict[str, Any] = {
            "kind": kind,
            "name": name,
        ...
        data = self._execute(_UPSERT_ENTITY, {"input": input})
        return EntityHandle._from_json(self, data["upsertEntity"])
    ...
    def update_entity(
        ...
        *,
        ...
        alone when omitted (or None). `node_path_parts`, `uri`, and `properties`
        are left alone when omitted; pass `node_path_parts=None` to detach the
        entity from its catalog node, or `uri=None` to clear the URI.
        """
        input: dict[str, Any] = {}
        if kind is not None:
            input["kind"] = kind
    ...
    def update_link(
        self, id: str, *, predicate: Optional[str] = None, properties: Any = _UNSET
    ) -> Optional["LinkHandle"]:
        """Update a link's predicate and/or properties by id; returns the handle.

        The predicate is left unchanged when omitted (or None); `properties` is
...
class LinkHandle:
    ...
    def update(
        self, *, predicate: Optional[str] = None, properties: Any = _UNSET
    ) -> "LinkHandle":
        "Update this link in place; see :meth:`GraphClient.update_link`."
        updated = self._graph.update_link(
            self.id, predicate=predicate, properties=properties
        )
        if updated is None:

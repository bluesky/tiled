...
def test_duckdb_data_types(duckdb_uri: str) -> None:
    schema = pa.schema([("some_int", pa.int32()), ("some_string", pa.string())])
    query = arrow_schema_to_create_table(schema, "random_test_table", "duckdb")
...
def test_psql_data_types(postgres_uri: str) -> None:
    print("uri", postgres_uri)
    schema = pa.schema([("some_int", pa.int32()), ("some_string", pa.string())])
    query = arrow_schema_to_create_table(schema, "random_test_table", "postgresql")

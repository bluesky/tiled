...
def context(tree):
    with Context.from_app(build_app(tree)) as context:
        ...
        x.write_array(time_1x, key="time_1x", metadata={}, specs=["xarray_coord"], dims=["time_1x"])
        x.write_array(time_2x, key="time_2x", metadata={}, specs=["xarray_coord"], dims=["time_2x"])
...
def test_read_xarray_same_shape(context):
    ...
    x.parts["tab1"].update_metadata(metadata={"column_specs":{"colA": ["xarray_data_var"]}, "rows_dim": "time_1x"})
    xarr = x.to_dataset(*keys).read()
    ...
    x.parts["tab1"].update_metadata(metadata={"column_specs": DELETE_KEY, "rows_dim": DELETE_KEY})

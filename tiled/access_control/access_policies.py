...
class InheritedTagAccessPolicy(TagBasedAccessPolicy):
    ...
    async def allowed_scopes(
        ...
        if self._is_admin(authn_scopes):
            return self.scopes
        if hasattr(node, "node") and node.node.parent is None:
            # The mount root has no ancestor to inherit tags from and is not
            # itself a taggable resource — it is just the entry point into
            # the hierarchy. Access control is enforced by its tagged
            # descendants (via `filters` and inheritance walks below), not
            # by the root itself.
            return self.scopes

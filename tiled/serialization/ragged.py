...
from tiled.media_type_registration import default_deserialization_registry, default_serialization_registry
...
from tiled.structures.ragged import RaggedStructure, OffsetArrayType, StartAndStopArraysType
...
def to_numpy_array(array: ragged.array) -> np.ndarray:
    ...
    while isinstance(content, (awkward.contents.ListOffsetArray, awkward.contents.ListArray)):
...
@default_serialization_registry.register(StructureFamily.ragged, "application/octet-stream")
...
def from_numpy_array(
    ...
    def rebuild(offsets: list[OffsetArrayType | StartAndStopArraysType]) -> awkward.contents.Content:
        nonlocal array
        ...
        if offsets:
            ...
            if len(indices) == 2 and isinstance(indices[0], Iterable):
                ...
                return awkward.contents.ListOffsetArray(offsets=awkward.index.Index(indices), content=rebuild(offsets))
        return awkward.contents.NumpyArray(array)

    return ragged.array(rebuild(offsets), dtype=dtype)


@default_deserialization_registry.register(StructureFamily.ragged, "application/octet-stream")
...
@default_serialization_registry.register(StructureFamily.ragged, media_type="application/zip")
def to_zipped_buffers(
    ...
    with zipfile.ZipFile(file, "w", compresslevel=zipfile.ZIP_STORED) as fzip:
        ...
        fzip.writestr("offsets", safe_json_dump(RaggedStructure.from_array(array).offsets))
        fzip.writestr("data", to_numpy_array(array).tobytes())
    return file.getvalue()


@default_deserialization_registry.register(StructureFamily.ragged, media_type="application/zip")
...
if modules_available("pyarrow"):
    ...
    @default_serialization_registry.register(StructureFamily.ragged, APACHE_ARROW_FILE_MIME_TYPE)

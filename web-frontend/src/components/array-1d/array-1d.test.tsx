...
describe("DataDisplay", () => {
  ...
  it("shows skeleton while loading", async () => {
    ...
    render(<DataDisplay name="foo" link="/api/array" range={[0, 2]} />);
    expect(screen.getByRole("progressbar")).toBeInTheDocument();
  });

  it("shows chart after data loads", async () => {
    mockGet.mockResolvedValue({ data: [1, 2, 3] });
    render(<DataDisplay name="foo" link="/api/array" range={[0, 3]} />);

    await waitFor(() => {
      expect(screen.getByTestId("mocked-chart")).toBeInTheDocument();
      expect(screen.getByText("MockedChart foo 0 1,2,3")).toBeInTheDocument();
    });
  });

...
# ---------------------------------------------------------------------------
# Tests: plain table (no Spec("xdi")) is rejected with a 406
# ---------------------------------------------------------------------------


def test_plain_table_cannot_export_as_xdi(client):
    """A table without Spec('xdi') must not be exportable as application/x-xdi.

    The server returns 406 Not Acceptable because 'application/x-xdi' is only
    registered for the 'xdi' spec, not for StructureFamily.table generally.
    """
    with pytest.raises(ClientError):
        _export_to_string(client["plain_table"], format="application/x-xdi")

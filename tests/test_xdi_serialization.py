...
The serializer is registered under StructureFamily.table and is intended for use
with tables tagged with Spec("xdi", ...). The primary use case is:

    c.write_table(df, metadata={...}, specs=[Spec("xdi")])
    table.export("output.xdi")
"""
...
def _build_tree():
    return MapAdapter(
        {
            "xdi_table": DataFrameAdapter.from_pandas(
                XDI_DF.copy(),
                npartitions=1,
                metadata=XDI_METADATA,
                specs=[Spec("xdi", version="1.0")],
            ),
            "xdi_no_comments": DataFrameAdapter.from_pandas(
                XDI_DF[["energy"]].copy(),
                npartitions=1,
            ...
            # export should still succeed since we register on StructureFamily.table.
...
def test_export_xdi_to_file(client, tmp_path):
    ...
    client["xdi_table"].export(out)
    assert out.exists()
    assert out.read_text().startswith("# XDI/")
...
def test_xdi_output_version_line(client):
    ...
    content = _export_to_string(client["xdi_table"], format="application/x-xdi")
    assert content.splitlines()[0] == "# XDI/1.0 GSE/1.0"
...
def test_xdi_output_defined_namespaces_before_extensions(client):
    ...
    content = _export_to_string(client["xdi_table"], format="application/x-xdi")
    assert content.find("# Element.") < content.find(
        "# GSE."
...
def test_xdi_output_separators_always_present_without_comments(client):
    """Field-end and header-end separators must appear even with no user comments."""
    content = _export_to_string(client["xdi_no_comments"], format="application/x-xdi")
    assert re.search(r"^# /{3,}", content, re.MULTILINE)
    assert re.search(r"^# -{3,}", content, re.MULTILINE)
...
def test_xdi_output_column_labels_after_header_end(client):
    ...
    content = _export_to_string(client["xdi_table"], format="application/x-xdi")
    assert content.find("# energy i0 itrans mutrans") > content.rfind(
        "# ----"
...
def test_xdi_output_returns_bytes(client):
    ...
    buf = io.BytesIO()
    client["xdi_table"].export(buf, format="application/x-xdi")
    buf.seek(0)
...
def test_xdi_roundtrip(client, tmp_path):
    """Output must be re-parseable by the XDI header parser (roundtrip integrity)."""
    ...
    for line in lines:
        ...
        if "." in key:
            ...
            fields[namespace.strip()][tag.strip()] = val.strip()
...
def test_xdi_format_alias(client):
    """Requesting format='xdi' should work the same as 'application/x-xdi'."""
...
# ---------------------------------------------------------------------------
# Tests: plain table (no Spec, no XDI metadata) can still be exported
# ---------------------------------------------------------------------------


def test_plain_table_exports_as_xdi(client):
    """Any table can be exported as XDI; a plain table produces a minimal valid file."""
    content = _export_to_string(client["plain_table"], format="application/x-xdi")
    # Must still have the structural skeleton
    assert content.splitlines()[0] == "# XDI/1.0"
    assert re.search(r"^# /{3,}", content, re.MULTILINE)
    assert re.search(r"^# -{3,}", content, re.MULTILINE)
    assert "# generated by tiled" in content
    # Column labels and data rows must still be present
    assert "# energy" in content
    data_lines = [ln for ln in content.splitlines() if ln and not ln.startswith("#")]
    assert len(data_lines) == len(XDI_DF)

...
def test_bind_entity_is_idempotent(client):
    "Re-binding the same (node, kind, name) returns the existing entity."
    client.write_array([1, 2, 3], key="array1")
    array1 = client["array1"]
    first = array1.bind_entity()
    second = array1.bind_entity()

    # Get-or-create: no error, same underlying entity, no duplicate row.
    assert first.id == second.id
    assert [e.id for e in array1.entities()] == [first.id]


def test_bind_entity_refreshes_properties(client):
    "Re-binding refreshes uri/properties on the existing entity."
    client.write_array([1, 2, 3], key="array1")
    array1 = client["array1"]
    first = array1.bind_entity(properties={"note": "first"})
    second = array1.bind_entity(properties={"note": "second"})

    assert first.id == second.id
    assert second.properties == {"note": "second"}


def test_graphclient_create_entity_still_conflicts(client):
    "GraphClient.create_entity (unlike bind_entity) still rejects duplicates."
    client.write_array([1, 2, 3], key="array1")
    array1 = client["array1"]
    graph = GraphClient(client.context)
    graph.create_entity(kind="array", name="array1", node_path_parts=array1.path_parts)

    with pytest.raises(EntityExistsError):
        graph.create_entity(
            kind="array", name="array1", node_path_parts=array1.path_parts
        )
...
def test_update_entity_properties(client):
    "update_entity can change properties, leaving them alone when omitted."
    sample = make_entity(
        client, name="LaB6", kind="sample", properties={"lattice": "cubic"}
    )
    graph = GraphClient(client.context)

    updated = graph.update_entity(sample.id, properties={"lattice": "hexagonal"})
    assert updated.properties == {"lattice": "hexagonal"}
    # Omitting properties leaves them in place.
    assert graph.update_entity(sample.id, name="renamed").properties == {
        "lattice": "hexagonal"
    }


def test_entity_handle_update_properties_in_place(client):
    "EntityHandle.update refreshes properties on the handle."
    sample = make_entity(client, name="LaB6", kind="sample")
    sample.update(properties={"k": "v"})
    assert sample.properties == {"k": "v"}


def test_update_link_properties(client, linked_pair):
    "update_link can change a link's properties as well as its predicate."
    _, _, link = linked_pair
    graph = GraphClient(client.context)

    updated = graph.update_link(link.id, properties={"confidence": 0.9})
    assert updated.properties == {"confidence": 0.9}
    # Updating only the predicate leaves properties in place.
    assert graph.update_link(link.id, predicate="prov:wasRevisionOf").properties == {
        "confidence": 0.9
    }


def test_link_handle_update_properties_in_place(client, linked_pair):
    "LinkHandle.update refreshes properties on the handle."
    _, _, link = linked_pair
    link.update(properties={"weight": 1})
    assert link.properties == {"weight": 1}
